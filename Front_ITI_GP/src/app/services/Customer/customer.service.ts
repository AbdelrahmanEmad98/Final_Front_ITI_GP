...
// ======================================================================
  public login(credentials: LoginDto): Observable<TokenDto> {
    return this.http.post<TokenDto>(this.API_URL+"Security/Login", credentials)
      .pipe(
        tap((tokenDto) => {
  ...
  public getTestData(): Observable<string[]> {
    return this.http.get<string[]>(
      this.API_URL+"Test/test"
    );
  }
  GetCustomerInfoById(CustomerId: any):Observable<any> {
    return this.http.get<any>
    (`https://localhost:7052/api/Customer/${CustomerId}`);
  }
}
